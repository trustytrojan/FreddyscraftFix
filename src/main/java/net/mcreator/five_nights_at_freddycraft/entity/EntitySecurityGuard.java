...
public class EntitySecurityGuard extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 45;
    public static final int ENTITYID_RANGED = 46;
    
    public EntitySecurityGuard(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 32);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "securityguard"), 45).name("securityguard").tracker(64, 3, true).egg(-16751002, -16777063).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.SecurityGuard.weight, ModConfig.SecurityGuard.minGroupSize, ModConfig.SecurityGuard.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/security.guard.png");
                }
            };
