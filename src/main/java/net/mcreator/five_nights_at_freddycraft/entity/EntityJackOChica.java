...
public class EntityJackOChica extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 77;
    public static final int ENTITYID_RANGED = 78;
    
    public EntityJackOChica(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 50);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "jackochica"), 77).name("jackochica").tracker(64, 3, true).egg(-39424, -6737152).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.JackOChica.weight, ModConfig.JackOChica.minGroupSize, ModConfig.JackOChica.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/jack_o_chica.png");
                }
            };
