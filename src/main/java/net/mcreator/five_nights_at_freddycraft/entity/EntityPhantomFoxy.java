...
public class EntityPhantomFoxy extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 53;
    public static final int ENTITYID_RANGED = 54;
    
    public EntityPhantomFoxy(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 36);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "phantomfoxy"), 53).name("phantomfoxy").tracker(64, 3, true).egg(-6711040, -13421824).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.PhantomFoxy.weight, ModConfig.PhantomFoxy.minGroupSize, ModConfig.PhantomFoxy.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/phantom_foxy.png");
                }
            };
