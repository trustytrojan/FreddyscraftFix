...
public class EntitySpringbonnie extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 87;
    public static final int ENTITYID_RANGED = 88;
    
    public EntitySpringbonnie(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 55);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "springbonnie"), 87).name("springbonnie").tracker(64, 3, true).egg(-26317, -6711040).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Springbonnie.weight, ModConfig.Springbonnie.minGroupSize, ModConfig.Springbonnie.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/springbonnie.png");
                }
            };
