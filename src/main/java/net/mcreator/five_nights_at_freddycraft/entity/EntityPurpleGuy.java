...
public class EntityPurpleGuy extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 43;
    public static final int ENTITYID_RANGED = 44;
    
    public EntityPurpleGuy(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 31);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "purpleguy"), 43).name("purpleguy").tracker(64, 3, true).egg(-3407668, -10092442).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.PurpleGuy.weight, ModConfig.PurpleGuy.minGroupSize, ModConfig.PurpleGuy.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/purple_guy.png");
                }
            };
