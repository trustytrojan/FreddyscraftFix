...
public class EntityNightmareBonnie extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 63;
    public static final int ENTITYID_RANGED = 64;
    
    public EntityNightmareBonnie(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 43);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "nightmarebonnie"), 63).name("nightmarebonnie").tracker(64, 3, true).egg(-10066177, -16777063).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.NightmareBonnie.weight, ModConfig.NightmareBonnie.minGroupSize, ModConfig.NightmareBonnie.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/nightmare_bonnie.png");
                }
            };
