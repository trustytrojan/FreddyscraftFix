...
public class EntityPhantomChica extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 51;
    public static final int ENTITYID_RANGED = 52;
    
    public EntityPhantomChica(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 35);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "phantomchica"), 51).name("phantomchica").tracker(64, 3, true).egg(-16711936, -16751104).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.PhantomChica.weight, ModConfig.PhantomChica.minGroupSize, ModConfig.PhantomChica.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/phantom_chica.png");
                }
            };
