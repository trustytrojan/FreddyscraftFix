...
public class EntityBidybab extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 109;
    public static final int ENTITYID_RANGED = 110;
    
    public EntityBidybab(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 73);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "bidybab"), 109).name("bidybab").tracker(64, 3, true).egg(-13159, -6749953).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Bidybab.weight, ModConfig.Bidybab.minGroupSize, ModConfig.Bidybab.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/bidybab.png");
                }
            };
