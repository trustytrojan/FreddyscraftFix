...
public class EntityCryingChild extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 93;
    public static final int ENTITYID_RANGED = 94;
    
    public EntityCryingChild(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 58);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "cryingchild"), 93).name("cryingchild").tracker(64, 3, true).egg(-16777216, -13382401).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.CryingChild.weight, ModConfig.CryingChild.minGroupSize, ModConfig.CryingChild.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/crying_child.png");
                }
            };
