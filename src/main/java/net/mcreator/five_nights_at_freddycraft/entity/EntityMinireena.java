...
public class EntityMinireena extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 113;
    public static final int ENTITYID_RANGED = 114;
    
    public EntityMinireena(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 75);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "minireena"), 113).name("minireena").tracker(64, 3, true).egg(-13159, -1).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Minireena.weight, ModConfig.Minireena.minGroupSize, ModConfig.Minireena.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/minireena.png");
                }
            };
