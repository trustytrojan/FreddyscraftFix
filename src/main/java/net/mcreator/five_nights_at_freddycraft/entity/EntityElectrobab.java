...
public class EntityElectrobab extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 111;
    public static final int ENTITYID_RANGED = 112;
    
    public EntityElectrobab(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 74);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "electrobab"), 111).name("electrobab").tracker(64, 3, true).egg(-13159, -205).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Electrobab.weight, ModConfig.Electrobab.minGroupSize, ModConfig.Electrobab.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/electrobab.png");
                }
            };
