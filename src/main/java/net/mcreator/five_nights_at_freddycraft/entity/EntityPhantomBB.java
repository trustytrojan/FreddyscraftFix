...
public class EntityPhantomBB extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 57;
    public static final int ENTITYID_RANGED = 58;
    
    public EntityPhantomBB(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 38);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "phantombb"), 57).name("phantombb").tracker(64, 3, true).egg(-6711040, -13421824).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.PhantomBB.weight, ModConfig.PhantomBB.minGroupSize, ModConfig.PhantomBB.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/phantom_bb.png");
                }
            };
