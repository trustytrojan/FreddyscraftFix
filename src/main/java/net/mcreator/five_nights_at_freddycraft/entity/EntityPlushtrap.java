...
public class EntityPlushtrap extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 73;
    public static final int ENTITYID_RANGED = 74;
    
    public EntityPlushtrap(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 48);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "plushtrap"), 73).name("plushtrap").tracker(64, 3, true).egg(-10040320, -3355648).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Plushtrap.weight, ModConfig.Plushtrap.minGroupSize, ModConfig.Plushtrap.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/plushtrap.png");
                }
            };
