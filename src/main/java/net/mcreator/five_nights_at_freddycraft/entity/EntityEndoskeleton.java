...
public class EntityEndoskeleton extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 11;
    public static final int ENTITYID_RANGED = 12;
    
    public EntityEndoskeleton(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 14);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "endoskeleton"), 11).name("endoskeleton").tracker(64, 3, true).egg(-13421773, -6710887).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Endoskeleton.weight, ModConfig.Endoskeleton.minGroupSize, ModConfig.Endoskeleton.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/endoskeleton.png");
                }
            };
