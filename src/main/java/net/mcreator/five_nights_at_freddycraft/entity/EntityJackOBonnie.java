...
public class EntityJackOBonnie extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 75;
    public static final int ENTITYID_RANGED = 76;
    
    public EntityJackOBonnie(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 49);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "jackobonnie"), 75).name("jackobonnie").tracker(64, 3, true).egg(-26368, -10079488).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.JackOBonnie.weight, ModConfig.JackOBonnie.minGroupSize, ModConfig.JackOBonnie.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/jack_o_bonnie.png");
                }
            };
