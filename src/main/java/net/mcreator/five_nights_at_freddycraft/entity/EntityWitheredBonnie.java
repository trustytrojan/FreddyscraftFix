...
public class EntityWitheredBonnie extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 27;
    public static final int ENTITYID_RANGED = 28;
    
    public EntityWitheredBonnie(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 23);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "witheredbonnie"), 27).name("witheredbonnie").tracker(64, 3, true).egg(-10092391, -3407872).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.WitheredBonnie.weight, ModConfig.WitheredBonnie.minGroupSize, ModConfig.WitheredBonnie.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/withered_bonnie.png");
                }
            };
