...
public class EntityWitheredChica extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 29;
    public static final int ENTITYID_RANGED = 30;
    
    public EntityWitheredChica(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 24);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "witheredchica"), 29).name("witheredchica").tracker(64, 3, true).egg(-256, -65485).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.WitheredChica.weight, ModConfig.WitheredChica.minGroupSize, ModConfig.WitheredChica.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/withered_chica.png");
                }
            };
