...
public class EntityNightmareMangle extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 79;
    public static final int ENTITYID_RANGED = 80;
    
    public EntityNightmareMangle(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 51);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "nightmaremangle"), 79).name("nightmaremangle").tracker(64, 3, true).egg(-1, -65383).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.NightmareMangle.weight, ModConfig.NightmareMangle.minGroupSize, ModConfig.NightmareMangle.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/nightmare_mangle.png");
                }
            };
