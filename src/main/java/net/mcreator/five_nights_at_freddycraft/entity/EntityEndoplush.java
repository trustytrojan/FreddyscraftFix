...
public class EntityEndoplush extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 91;
    public static final int ENTITYID_RANGED = 92;
    
    public EntityEndoplush(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 57);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "endoplush"), 91).name("endoplush").tracker(64, 3, true).egg(-6710887, -10066330).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.Endoplush.weight, ModConfig.Endoplush.minGroupSize, ModConfig.Endoplush.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/endoplush.png");
                }
            };
