...
public class EntityShadowFreddy extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 37;
    public static final int ENTITYID_RANGED = 38;
    
    public EntityShadowFreddy(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 28);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "shadowfreddy"), 37).name("shadowfreddy").tracker(64, 3, true).egg(-6750055, -3394561).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.ShadowFreddy.weight, ModConfig.ShadowFreddy.minGroupSize, ModConfig.ShadowFreddy.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/shadow_freddy.png");
                }
            };
