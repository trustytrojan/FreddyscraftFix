...
public class EntityNightmareFoxy extends ElementsFiveNightsAtFreddycraftMod.ModElement
{
    public static final int ENTITYID = 67;
    public static final int ENTITYID_RANGED = 68;
    
    public EntityNightmareFoxy(final ElementsFiveNightsAtFreddycraftMod instance) {
        super(instance, 45);
    }
    
    @Override
    public void initElements() {
        this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("five_nights_at_freddycraft", "nightmarefoxy"), 67).name("nightmarefoxy").tracker(64, 3, true).egg(-52429, -10092493).build());
    }
    
    @Override
    public void init(final FMLInitializationEvent event) {
        final Biome[] spawnBiomes = this.allbiomes((RegistryNamespaced<ResourceLocation, Biome>)Biome.REGISTRY);
        EntityRegistry.addSpawn(EntityCustom.class, ModConfig.NightmareFoxy.weight, ModConfig.NightmareFoxy.minGroupSize, ModConfig.NightmareFoxy.maxGroupSize, EnumCreatureType.MONSTER, spawnBiomes);
    }
        ...
        RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, renderManager -> {
            final RenderBiped customRender = new RenderBiped(renderManager, new ModelBiped(), 0.5f) {
                protected ResourceLocation getEntityTexture(final Entity entity) {
                    return new ResourceLocation("five_nights_at_freddycraft:textures/nightmare_foxy.png");
                }
            };
